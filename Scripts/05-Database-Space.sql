...
BEGIN
    ...
    SELECT
        ...
        CAST((size-FILEPROPERTY(name, ''SpaceUsed''))/128.0 AS decimal(18,2)),
        CAST(CASE WHEN size = 0 THEN 0 ELSE ((size-FILEPROPERTY(name, ''SpaceUsed''))*100.0/size) END AS decimal(18,2)),
        CASE WHEN is_percent_growth = 1
             THEN CAST(growth AS varchar(20)) + ''%''
             ELSE CAST(CAST(growth/128.0 AS decimal(18,2)) AS varchar(30)) + '' MB'' END
    ...
    END TRY
    BEGIN CATCH
        PRINT CONCAT('Could not inspect database ', QUOTENAME(@db), ': ', ERROR_MESSAGE());
    END CATCH;
    FETCH NEXT FROM dbs INTO @db;
END
CLOSE dbs;
...
SELECT *,
    CASE WHEN FreePercent < @FreeSpaceWarningPercent THEN 'WARNING' ELSE 'PASS' END AS Severity,
    CASE WHEN FreePercent < @FreeSpaceWarningPercent
         THEN 'Low free space inside the database file. Review growth pattern, autogrowth settings, and underlying volume capacity.'
         ELSE 'File has free space above the configured threshold.' END AS RecommendedAction
FROM #FileSpace
ORDER BY FreePercent, DatabaseName, LogicalFileName;
